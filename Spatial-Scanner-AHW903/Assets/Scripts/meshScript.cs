...
#pragma warning restore CS4014
            ...
            if (QuerySceneFromDevice)
            {
                if (AutoRefresh)
                {
                    ...
                    if (TimeElapsedSinceLastAutoRefresh >= AutoRefreshIntervalInSeconds)
                    {
                        ...
                        {
                            Debug.LogError($"Error in {nameof(meshScript)} {nameof(AutoRefresh)}: {ex.Message}");
                        }
        ...
        public Task DisplayDataAsync()
        {
            ...
            if ((displayTask != null) && (!displayTask.IsCompleted))
            {
                ...
                Debug.Log($"{nameof(meshScript)}.{nameof(DisplayDataAsync)} already in progress.");
                return displayTask;
            }
...
#pragma warning restore CS1998 // Async method lacks 'await' operators and will run synchronously
        {
            ...
            if (QuerySceneFromDevice)
            {
                string fileName = string.Format("AHW903_{0}-{1}-{2}_{3}-{4}-{5}.bytes",
                                                year, month, day, hour, min, sec);

                byte[] OnDeviceBytes = GetLatestSceneBytes();

#if WINDOWS_UWP
                var file = await WindowsStorage.DownloadsFolder.CreateFileAsync(fileName, WindowsStorage.CreationCollisionOption.GenerateUniqueName);
            ...
            else
            {
                ...
                foreach (TextAsset serializedScene in SUSerializedScenePaths)
                {
                    ...
                    string fileName = string.Format("AHW903_Frag{0}-{1}-{2}-{3}_{4}-{5}-{6}.bytes",
...
#pragma warning restore CS1998 // Async method lacks 'await' operators and will run synchronously
        ...
        #region Created Public Functions
 
        public async void toggleMesh()
        {
            Debug.Log("Toggle Mesh Button Pressed.");
            FilterWorldMesh = !FilterWorldMesh;
            await DisplayDataAsync();
        }
        ...
        public void saveData()
        {
            ...
            var bytes = SaveBytesToDiskAsync();
            var objs = SaveObjsToDiskAsync();
